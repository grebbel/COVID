...
## possible use later
iso3_cnames <- read_csv("https://github.com/kjhealy/covid/raw/master/data/countries_iso3.csv")
iso2_to_iso3 <- read_csv("https://github.com/kjhealy/covid/raw/master/data/iso2_to_iso3.csv")
...
covid_raw  <- read_excel(tf)


# 
# covid_raw <- get_ecdc_data(url = "https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-2020-03-22.xlsx",
#                            fname = "COVID-19-geographic-distribution-worldwide-",
#                            ext = "xlsx")
covid_raw
# covid <- covid_raw %>%
...
cov_curve %>%
  ...
  labs(x = "Days Since 10th Confirmed Death", 
       ...
       title = "Cumulative Reported Deaths from COVID-19, Selected Countries", 
...
us_states %>%
  ...
  labs(title = "Cumulative Cases to Date, by State",
       y = "Cases", 
       x = "Date") + 
  annotate("text", x = lubridate::ymd("2020-03-19"), y = 10500, 
           label = "NY", color = "red") + 
  theme(axis.text.y = element_text(size = rel(2)))
```